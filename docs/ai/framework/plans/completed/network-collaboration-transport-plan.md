...
Completed and approved for closeout on 2026-07-23. The former Yjs operation-log
design remains superseded by this transport-only contract because application
publications, not a framework-owned CRDT history, are the canonical unit of
collaboration.

This completed record does not authorize beginning Framework Release Gate 3.

## Completion Record

- Final decision: keep Collaboration optional, provider-replaceable, live-only,
  publication-based, and free of app semantic policy or retained history.
- Implementation summary: Factory publications now pass through one explicit
  Collaboration lifecycle and replaceable Provider into one app-owned inbound
  callback; Awareness remains a separate ephemeral route, and Asyra Design
  supplies the real open-source WebSocket reference composition.
- Validation snapshot: at baseline `fb9dece1a`, repository tests passed 17/17
  tasks, the dedicated Inspector passed 28/28 contracts, collaboration browser
  E2E passed 4/4 cases, collaboration/server and workspace production builds
  passed, and lint reported zero errors. The product owner also confirmed the
  real multi-window CRDT behavior was operating normally.
- Exit criteria: public APIs, lifecycle, transport, app canonical apply,
  reconnect, Awareness, documentation, and direct consumers match this
  transport-only contract; the product owner approved closeout on 2026-07-23.
