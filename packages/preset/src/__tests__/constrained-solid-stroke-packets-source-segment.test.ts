...
describe('constrained solid stroke packets: source-segment adherence and smooth corners', () => {
  it('should run: locate right-bottom internal pentagon source-segment adherence layer owner', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-bottom-source-segment-layer-owner`,
      ...
      const layerCoverage =
        analyzeInsideSolidRightBottomSourceSegmentLayerCoverageForTest({
          backend,
          entry,
          sourcePath,
          join: joinType,
      ...
      expect(
        layerCoverage.targetSegmentRange,
  ...
  it('should run: keep source-segment adherence independent of source segment index', async () => {
    const baseline = buildSelfCheckStarSolidDomainFixture()
    const baselineTargetFrame = getSegmentProbeFrameForTest(
      baseline.sourcePath,
      getSourcePathSegmentRangesForTest(baseline.sourcePath)[3],
      0.676
    )
    expect(baselineTargetFrame).toBeTruthy()
    if (!baselineTargetFrame) {
      return
    }
    ...
    const backend = createClipper2GeometryBackend(await loadClipperModule())
    const cases = ['miter', 'bevel', 'round'] as const
    const targetSegmentEndpoints =
      getSelfCheckRightBottomSourceSegmentEndpointsForTest()

    expect(sourcePath.segments[0]?.start).toEqual({
      x: 0,
      y: 15.668954151283657
    })
    ...
    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:source-segment-adherence-reindexed`,
        topology.normalizedPoints,
        true,
        [
          createDefaultStroke({
      ...
      expectInsideSolidRightBottomSourceSegmentAdherenceForTest({
        name: `inside solid ${joinType} reindexed visible geometry`,
        polygons: visiblePolygons,
        sourcePath,
        strokeWidth: 10,
        targetPoint: baselineTargetFrame.point,
  ...
  it('should run: keep inside solid round internal pentagon corners as single smooth lobes', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    const backend = createClipper2GeometryBackend(await loadClipperModule())
    const packets = buildConstrainedSolidStrokeResolvedPackets(
      'self-check-star-solid-domain:inside:round:internal-corner-lobes',
      topology.normalizedPoints,
      true,
      [
        createDefaultStroke({
