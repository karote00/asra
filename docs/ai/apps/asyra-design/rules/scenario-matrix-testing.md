# Rule: Scenario-Matrix Testing

## Purpose

All geometry-producing or visually observable rendering features must be
validated from a scenario matrix first, not from incident screenshots first.
...
4. write visual tests for promoted scenarios
...
Visual tests validate what the user actually sees for promoted slices.

Visual tests must:

- go through the real app/runtime path
- cover representative promoted scenarios from each scenario family
