...
- **Package**: `@asyra/input-system` (or its adapter)
- **Action**: It captures the browser's raw **mouse events** (e.g., `mousedown`, `mouseup`).

- **Package**: `@asyra/core` (acting as middleware)
...
- **Event Published**: The `core` package publishes a generic **Input Action** event, such as `executeAction` (via `@asyra/reactive-events`), carrying the context and details of the interaction.
...
The `@asyra/core` package acts as the central coordinator. It hears the decision and executes the necessary sequence of operations.
...
-   **Transaction Update**: As the change is applied to the `scene-tree`'s state, `scene-tree` calls `updateTransaction()` (via `@asyra/reactive-events`) with the relevant payload describing the change. This records the specific modification for the undo stack.
-   **Event Published**: The `scene-tree` publishes a `sceneTreeChanged` event (via `@asyra/reactive-events`) to announce that its state has been updated. This event serves as a high-level notification.
...
-   **Action**: `core` proceeds with the next steps in the user action flow:
    ...
    -   It calls `selectElements()` (via `@asyra/reactive-events`) to ensure the newly created element is selected.
    -   Finally, it calls `endTransaction()` (via `@asyra/reactive-events`) to commit all recorded changes for this user action as a single undoable unit.
...
-   **Action**: The `render` package *primarily* observes the YJS resource (which is the source of truth for the scene graph) *managed by the `@asyra/factory`* for granular data changes. When the YJS object changes, `render` automatically updates its visual representation. It *also* observes a separate **YJS Selection Object** (also managed by `@asyra/factory`) to determine which elements are currently selected. Based on this, it draws appropriate selection boxes around the selected elements. Furthermore, `render` listens to relevant events from `@asyra/reactive-events` (such as `subscribeToPanTo`) for commands or signals that are not directly tied to YJS data changes, but trigger specific rendering actions.
...
-   **Action**: The `ui-context` package *primarily* observes the YJS resource *managed by the `@asyra/factory`* for UI-relevant data. It *also* observes the separate **YJS Selection Object** to determine which elements are selected. Based on the selected elements, `ui-context` performs **property aggregation**: for each property (e.g., x, y, width, height), it checks if all selected elements have the same value. If so, that value is saved. If not, the value is set to `MIXED_STRING` (from `@asyra/utils`). These aggregated properties are exposed as `rxjs BehaviorSubject`s. Furthermore, `ui-context` listens to relevant events from `@asyra/reactive-events` for commands or signals that are not directly tied to YJS data changes, but trigger specific UI actions.
-   **Response**: When the YJS objects change, or relevant events are received, `ui-context` automatically processes these changes to update its internal UI state. UI components (in `apps/asyra-design`) subscribe to the `BehaviorSubject`s exposed by `ui-context`, ensuring that only relevant input fields are updated when a property changes, and the UI is synchronized with the underlying data.

#### Step 9: User Interface Updates (apps/asyra-design)
...
-   **Action**: The UI components within `apps/asyra-design` subscribe to the `BehaviorSubject`s exposed by the `@asyra/ui-context` package. These subscriptions allow the UI to react efficiently to granular changes in aggregated properties or other UI-specific state.
