...
export type StrokeGeometryOracleGroup =
...
export interface StrokeGeometryOracleCoverageCase {
  id: string
  title: string
  oracleGroup: StrokeGeometryOracleGroup
  ...
  testSourceFiles?: readonly string[]
...
const reportedVector34SeamOracleFiles = [
  oracle('reported-vector-34-seam-oracle.test.ts'),
  oracle('reported-vector-34-seam-microscope-miter-oracle.test.ts'),
  oracle('reported-vector-34-seam-microscope-bevel-oracle.test.ts'),
  oracle('reported-vector-34-seam-microscope-round-oracle.test.ts'),
  oracle('reported-vector-34-seam-footprint-classes-oracle.test.ts'),
  oracle('reported-vector-34-seam-terminal-residue-oracle.test.ts'),
  oracle('reported-vector-34-seam-join-connectivity-oracle.test.ts')
] as const

const reportedVector34TerminalOracleFiles = [
  oracle('reported-vector-34-terminal-oracle.test.ts'),
  oracle('reported-vector-34-terminal-survival-oracle.test.ts'),
  oracle('reported-vector-34-terminal-reference-oracle.test.ts')
] as const

const reportedVector34OutputOracleFiles = [
  oracle('reported-vector-34-output-oracle.test.ts'),
  oracle('reported-vector-34-output-outside-legality-oracle.test.ts'),
  oracle('reported-vector-34-output-shared-boundary-oracle.test.ts'),
  oracle('reported-vector-34-output-sequential-update-oracle.test.ts'),
  oracle('reported-vector-34-output-full-diagnostics-oracle.test.ts'),
  oracle('reported-vector-34-output-metadata-ownership-oracle.test.ts')
] as const
...
const rawStrokeGeometryOracleCoverageMap: readonly Omit<
  StrokeGeometryOracleCoverageCase,
  'focusedGates'
>[] = [
  {
    id: 'stroke-parameter-normalization-matrix',
    oracleGroup: 'normalization-domain',
    artifactChannels: ['stage'],
    title:
  ...
  },
  {
    ...
    oracleGroup: 'center-product',
    artifactChannels: ['internal', 'stage'],
    title:
  ...
  },
  {
    ...
    oracleGroup: 'center-product',
    artifactChannels: ['hit-export', 'render', 'render-hit-export', 'stage'],
    title:
  ...
  },
  {
    ...
    oracleGroup: 'constrained-product',
    artifactChannels: ['internal'],
    title:
  ...
  },
  {
    ...
    oracleGroup: 'legality-final-face',
    artifactChannels: ['internal', 'render', 'render-hit-export'],
    title:
    ...
    testSourceFiles: [oracle('legality-clipping-runtime-oracle.helpers.ts')],
    testNames: [
  ...
  },
  {
    ...
    oracleGroup: 'constrained-product',
    artifactChannels: ['evidence', 'internal'],
    title:
    ...
    testFiles: [oracle('constrained-product-family-oracle.test.ts')],
    testNames: [
      'keeps constrained dashed body, terminal, and smooth ownership in separate visible and evidence classes'
    ],
    positiveAssertions: [
      'Step 27 owns the visible constrained dashed body while Step 30 and Step 31 preserve terminal and smooth ownership as separate zero-visible evidence overlays.'
    ],
    forbiddenContributors: [
      'bridge',
  ...
  },
  {
    ...
    productFamily: ['constrained-dashed-product', 'dash-interval-body-product'],
  ...
  },
  {
    ...
    oracleGroup: 'legality-final-face',
    artifactChannels: ['internal', 'render', 'render-hit-export', 'stage'],
    title:
  ...
  },
  {
    ...
    testFiles: [
      ...
      ...reportedVector34SeamOracleFiles,
  ...
  },
  {
    ...
    testFiles: [
      ...
      oracle('ordinary-sharp-runtime-oracle.test.ts'),
      ...reportedVector34SeamOracleFiles,
      ...reportedVector34OutputOracleFiles
    ],
    testNames: [
    ...
    positiveAssertions: [
      'Source-vertex join products share the same seam-boundary artifact endpoint identities as incident dash body seams; visible source-space seam gap is zero.',
      'Source-vertex join products and final faces share the full seam-boundary artifact terminal-to-outer-endpoint seam edge, not only the outer endpoint.',
      'Step 28 terminal seam-boundary evidence preserves the full inner-to-outer stroke-width edge before Step 29 source-vertex join consumption.',
      'Step 39 render entries either preserve the seam edge or declare a single-paint render-projection merge that keeps seam endpoint/midpoint coverage provenance without repeated-alpha overdraw.',
      'Step 39 same-paint render entries with shared or near-shared source-space boundaries are merged before renderer projection to prevent high-zoom antialias seams.',
      'Step 39 same-paint render entries do not carry internally shared-boundary polygons into renderer projection; touching products are fused into canonical projection polygons while disjoint dash products remain separate.',
      ...
      'Step 28 seam-boundary artifacts, Step 29 source-vertex joins, Step 33 legality records, Step 36 final faces, and Step 39 render entries keep identical source-space product signatures across viewport zoom states.'
    ],
    forbiddenContributors: [
  ...
  },
  {
    ...
    artifactChannels: ['evidence', 'internal'],
    title:
      'Step 27 owns dash cap geometry while Step 30 terminal evidence stays non-visible and cannot become join repair',
    caseKind: 'forbidden-contributor',
    coverageStrategy: 'spec-critical-higher-order',
    strokeParameters: [
      'style:dashed',
    ...
    testNames: [
      'keeps cap geometry on Step 27 bodies while Step 30 terminal ownership stays non-visible'
    ],
    positiveAssertions: [
      'Cap contributors appear only on Step 27 dash body products; Step 30 preserves terminal ownership as zero-visible evidence.',
  ...
  },
  {
    ...
    oracleGroup: 'constrained-product',
    artifactChannels: ['evidence'],
    title:
      'Smooth high-curvature dashed spans keep Step 27 visible body ownership and Step 31 non-visible continuity evidence',
    caseKind: 'forbidden-contributor',
    coverageStrategy: 'spec-critical-higher-order',
    strokeParameters: [
      'style:dashed',
    ...
    ownerStages: ['Stroke Geometry smooth-continuity ownership binding'],
    stepIds: [
      'build-source-vertex-join-products',
      'build-smooth-continuity-products'
    ],
    ...
    testFiles: [
      oracle('join-dash-product-oracle.test.ts'),
      oracle('reported-vector-34-smooth-oracle.test.ts')
    ],
    testNames: [
    ...
    positiveAssertions: [
      'High-curvature tangent-continuous constrained dashed spans remain Step 27 body products while Step 31 emits a non-visible evidence overlay, not a sharp join or duplicate body.'
    ],
    forbiddenContributors: [
  ...
  },
  {
    ...
    oracleGroup: 'output-channel',
    artifactChannels: ['hit-export', 'render', 'render-hit-export'],
    title:
    ...
    positiveAssertions: [
      ...
      'Final face metadata stays the source for render and hit/export sibling channels before post-runtime validation reads terminal evidence.'
    ],
    forbiddenContributors: [
  ...
  },
  {
    ...
    oracleGroup: 'dash-cap-join',
    artifactChannels: ['evidence', 'internal', 'render', 'render-hit-export'],
    title:
    ...
    testFiles: [
      ...reportedVector34SeamOracleFiles,
      ...reportedVector34TerminalOracleFiles,
      ...reportedVector34OutputOracleFiles,
      oracle('reported-vector-34-smooth-oracle.test.ts')
    ],
    testNames: [
    ...
    positiveAssertions: [
      ...
      'Reported vector-34 source-vertex joins consume Step 28 terminal seam-boundary artifact edges whose inner-to-outer distance remains the authored stroke width.',
      'Step 39 render-entry materialization keeps shared seam-boundary artifact coverage visible either as a preserved seam edge or as a declared single-paint render-projection merge with seam coverage provenance.',
      ...
      'Step 30 terminal ownership remains zero-visible evidence over Step 27 body products and preserves overlay identity through final faces, render entries, hit packets, and export packets.',
      'High-curvature constrained dashed spans preserve Step 31 non-visible smooth ownership evidence across packet, final-face, render, hit, and export channels while Step 27 remains the visible body owner.',
  ...
  },
  {
    ...
    oracleGroup: 'output-channel',
    artifactChannels: ['internal', 'render-hit-export'],
    title:
    ...
    testFiles: [
      'packages/preset/src/__tests__/stroke-flow-integration/render-mirror-current-state-cache.test.ts'
    ],
    testNames: [
...
export const strokeGeometryOracleCoverageMap: readonly StrokeGeometryOracleCoverageCase[] =
  rawStrokeGeometryOracleCoverageMap.map((entry) => ({
    ...entry,
    focusedGates: entry.testNames.map(
      (testName) =>
