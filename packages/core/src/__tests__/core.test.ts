...
vi.mock('@asra/input-system', () => {
  const mockKeyMapInstance = {
    ...
    isSpecialEvent: vi.fn((event: SpecialEvent) => event === SpecialEvent.WHEEL),
    keys: {},
  };
...
describe('Core', () => {
  ...
  let mockInputSystem: vi.Mocked<InstanceType<typeof InputSystemModule.InputSystem>>;
  ...
  let mockPropsManager: vi.Mocked<InstanceType<typeof PropsManagerModule.PropsManager>>;
  let mockSystemContext: vi.Mocked<InstanceType<typeof SystemContextModule.SystemContext>>;
  let mockInteractionCore: vi.Mocked<InstanceType<typeof InteractionCoreModule.InteractionCore>>;
  ...
  beforeEach(() => {
    ...
    mockInputSystem = {
      ...
      on: vi.fn(),
      switchWatchedElement: vi.fn(),
    } as unknown as vi.Mocked<InstanceType<typeof InputSystemModule.InputSystem>>;
    ...
    mockPropsManager = {
      load: vi.fn(),
      save: vi.fn(),
    } as unknown as vi.Mocked<InstanceType<typeof PropsManagerModule.PropsManager>>;
    ...
    mockSystemContext = {
      ...
      updateMouseState: vi.fn(),
      updateKeyState: vi.fn(),
    } as unknown as vi.Mocked<InstanceType<typeof SystemContextModule.SystemContext>>;
    ...
    mockInteractionCore = {
      ...
      updateSession: vi.fn(),
      endSession: vi.fn(),
    } as unknown as vi.Mocked<InstanceType<typeof InteractionCoreModule.InteractionCore>>;
  ...
  it('should load data correctly and call appropriate methods', () => {
    const mockSceneTreeData = { elements: {}, workspace: 'ws-1', workspaceList: ['ws-1'] };
  ...
  it('should delegate switchPrimaryTool to systemContext', () => {
    ...
    expect(mockApis.switchPrimaryTool).toHaveBeenCalledWith(PrimaryToolType.RECTANGLE);
  });
