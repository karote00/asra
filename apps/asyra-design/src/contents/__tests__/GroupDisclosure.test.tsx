...
describe('Layers Group disclosure', () => {
  ...
  it('exposes expansion state and toggles UI-local presentation only', () => {
    ...
    expect(iconContainer?.classList.contains('h-3')).toBe(true)
    expect(iconContainer?.classList.contains('w-3')).toBe(true)
    expect(iconContainer?.classList.contains('[&>svg]:h-3')).toBe(true)
    expect(iconContainer?.classList.contains('[&>svg]:w-3')).toBe(true)
