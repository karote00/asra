...
const buildSourcePathSampleMetadata = (
...
const applyInsideSolidSharpGuardClippingToEntries = (
  ...
  const endpointReach = sourcePath
    ? Math.max(stroke.width * 1.5, 1)
    : reach
...
export const buildSelfIntersectingClosedConstrainedStrokePolygonsForDashedApproximation =
  (
    points: Vec2[],
    stroke: Pick<
      RenderableStroke,
    ...
    if (!supportsConstrainedSolidStroke(stroke, true)) {
      return []
    }
