...
-   **Dispatch**: The handler for `INTERACTION_UNDO` publishes the high-level event `undo()` via `@asyra/reactive-events`.
...
-   **Action**:
    ...
    5.  It **re-publishes** `REMOVE_ELEMENT` (with the element's ID) via `@asyra/reactive-events`.
...
-   **Package**: `@asyra/render` & `@asyra/ui-context`
