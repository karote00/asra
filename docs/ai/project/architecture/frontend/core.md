...
The package follows a **Facade & Mediator Pattern** with Request-Response architecture.

- **`core.ts`**: The main entry point. It exports the `Core` class, which aggregates all APIs. It initializes the system by injecting dependencies (`inputSystem`, `factory`, `render`, etc.) into the subscribers.
- **`apis/`**: This directory contains the implementation of the high-level APIs. These functions orchestrate **request APIs** to execute business workflows.
  - _Example_: The `scene-tree.ts` implementation of `addRectangle` uses request APIs: calls `startTransaction()`, then `requests.sceneTree.addRectangle()`, then `requests.selection.selectElements()`, then `endTransaction()`. This encapsulates the business process using synchronous data access.
- **`requests/`**: This directory contains **Request APIs** that provide direct synchronous access to package state without business logic.
  - _Example_: `requests/scene-tree.ts` provides `sceneTreeSaveData()` and `addRectangle()` methods that directly interact with scene-tree state.
