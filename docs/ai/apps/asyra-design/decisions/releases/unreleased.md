...
## 2026-07-24 - Official Group canonical bounds become hoverable

- Context:
  - An invisible Group had a valid canonical hover/selection outline, but after
    its visible child moved within the Group, the empty part of the retained
    Group bounds produced no raw Render hit and therefore no hover target.
- Decision:
  - Keep visible raw Render hits as first priority.
  - When no raw hit exists and no `Meta`/`Ctrl` modifier is active, let the app
    test official Group canonical computed bounds through each Group's current
    identity-safe Render transform and resolve that candidate through the
    existing hierarchy scope policy.
  - Keep this candidate hover-only. Click selection, pointer-down move, and
    create-parent resolution continue using identity-safe raw Render hits.
- Consequences:
  - Empty retained Group bounds now show the ordinary hover outline.
  - Modifier access still reaches only an actually hit non-Group element.
  - Render gains no Group hit geometry or hierarchy policy, and the app keeps
    no second bounds or hierarchy state.
- Related Commit(s):
  - pending
