...
test.describe('Asyra Design Group interaction MVP', () => {
  ...
  test('hovers an official Group through empty canonical bounds after its child moves', async ({
    page
  }) => {
    await page.goto('/')
    await waitForAppReady(page)
    ...
    const rectId = (await getSelectedIds(page))[0]
    const originalCenter = await getSelectedElementClientCenter(page)
    expect(originalCenter).not.toBeNull()
    if (!originalCenter) {
      return
    }
    ...
    const innerGroupId = await groupLayerIds(page, [rectId])
    const outerGroupId = await groupLayerIds(page, [innerGroupId])
    const groupPositionsBeforeMove = await page.evaluate(
      ({ innerId, outerId }) => {
        const sceneTree = window.__Core__.deps.sceneTree
        return [innerId, outerId].map((elementId) => {
          const computed =
      ...
      { innerId: innerGroupId, outerId: outerGroupId }
    )

    await layerRow(page, rectId).click()
    await page.mouse.move(originalCenter.x, originalCenter.y)
    await page.mouse.down()
    await page.mouse.move(originalCenter.x + 160, originalCenter.y + 120, {
      steps: 4
    })
    await page.mouse.up()

    const groupPositionsAfterMove = await page.evaluate(
      ({ innerId, outerId }) => {
        const sceneTree = window.__Core__.deps.sceneTree
        return [innerId, outerId].map((elementId) => {
          const computed =
    ...
    expect(groupPositionsAfterMove).toEqual(groupPositionsBeforeMove)
    ...
    await page.keyboard.down('Meta')
    try {
      await page.mouse.move(originalCenter.x + 1, originalCenter.y)
