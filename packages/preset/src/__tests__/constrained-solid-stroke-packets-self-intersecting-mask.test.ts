...
describe('constrained solid stroke packets: self-intersecting mask model', () => {
  it('should run: require self-intersecting inside solidMaskModel packets, not boundary-domain ribbon products', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    const exportPolygons = buildSolidCenterStrokeExportPackets(packets).flatMap(
      (packet) => packet.polygons
    )
    const hitPolygons = buildSolidCenterStrokeHitTestPackets(packets).flatMap(
      (packet) => packet.polygons
    )
    const finalFacePolygons = finalFaces.flatMap((face) => face.polygons)
    const farSourceCoverageOptions = {
      sourcePath,
      maxDistance: 64,
      step: 4
    }
    ...
    expect(
      packets.every(
        (packet) =>
          ...
          packet.geometry.debugMeta?.solidMaskModelMaskSide === 'inside-fill'
      ),
      JSON.stringify(
    ...
    expect(
      getFarSourceCoverageFailures({
        polygons: packetPolygons,
        ...farSourceCoverageOptions
      }),
      JSON.stringify({ polygonCount: packetPolygons.length }, null, 2)
    ).toEqual([])
    expect(
      getFarSourceCoverageFailures({
      ...
      JSON.stringify({ finalFaceCount: finalFaces.length }, null, 2)
    ).toEqual([])
    expect(
      getFarSourceCoverageFailures({
      ...
      JSON.stringify({ exportPolygonCount: exportPolygons.length }, null, 2)
    ).toEqual([])
    expect(
      getFarSourceCoverageFailures({
        polygons: hitPolygons,
        ...farSourceCoverageOptions
      }),
      JSON.stringify({ hitPolygonCount: hitPolygons.length }, null, 2)
    ).toEqual([])
    const renderEntries =
  ...
  it('should run: enforce inside solid adjacency half-width probes on self-check star', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    const { sharedEdge, normalEdge } =
      chooseInsideSolidAdjacencyProbeEdgesForTest(legalFaceBoundaries)
    const probePairs =
      getInsideSolidAdjacencyProbePairsForTest(legalFaceBoundaries)

    expect(topology.topologyFamily).toBe('self-intersecting')
    expect(sharedEdge).toBeTruthy()
    expect(normalEdge).toBeTruthy()
    expect(probePairs.length).toBeGreaterThan(0)

    const phaseDurations = new Map<string, number>()
    const packetsStart = performance.now()
    const packets = withStrokeDiagnosticsMode('full', () =>
      withVectorRenderPhaseSink(
        (phaseName, durationMs) => {
          addPhaseDuration(phaseDurations, phaseName, durationMs)
        },
        () =>
          buildConstrainedSolidStrokeResolvedPackets(
            'self-check-star-solid-domain:inside-adjacency',
            topology.normalizedPoints,
            true,
            [
              createDefaultStroke({
    ...
    const packetsMs = performance.now() - packetsStart
    const finalFaces = buildStrokeFinalFacesFromResolvedPackets(packets)
    const renderEntries =
      toSolidCenterStrokeRenderEntriesFromFinalFaces(finalFaces)
    const packetPolygons = packets.flatMap((packet) => packet.geometry.polygons)
    ...
    const slowPhases = [...phaseDurations.entries()].filter(
      ([phaseName, durationMs]) =>
        phaseName.includes('solid-mask-model') && durationMs > 150
    )
    expect(packets.length).toBeGreaterThan(0)
    expect(
      packetsMs,
      JSON.stringify(
        {
          packetsMs,
          phaseDurations: Object.fromEntries(phaseDurations)
        },
        null,
        2
      )
    ).toBeLessThan(350)
    expect(slowPhases, JSON.stringify(slowPhases, null, 2)).toEqual([])
    expect(renderClipPolygons.length).toBeLessThanOrEqual(80)
    expect(sumPolygonPointCount(renderClipPolygons)).toBeLessThanOrEqual(2500)
    expect(
      renderEntries.every(
        (entry) => (entry.strokeMaskPolygons?.length ?? 0) === 0
      )
    ).toBe(true)
    expect(
      renderEntries.every(
    ...
    expect(
      packets.every((packet) => {
        const meta = packet.geometry.debugMeta as
          | (NonNullable<typeof packet.geometry.debugMeta> & {
              solidMaskModelInsideMaskMode?: string
              solidMaskModelVisibleMaskMode?: string
              solidMaskModelJoinGeometrySource?: string
        ...
        return (
          ...
          packet.geometry.debugMeta?.solidMaskModelVisibleRender ===
            'masked-source-stroke' &&
          packet.geometry.debugMeta?.solidMaskModelMaskSide === 'inside-fill' &&
          meta?.solidMaskModelInsideMaskMode === 'face-occupancy-inside-fill' &&
          meta?.solidMaskModelVisibleMaskMode ===
            'inside-fill-source-stroke-clip' &&
          meta?.solidMaskModelJoinGeometrySource ===
            'authored-doubled-source-stroke' &&
          probeNames.every((probeName) =>
            meta?.solidMaskModelAdjacencyProbe?.includes(probeName)
          ) &&
          !solidPacketHasDashedTerminalMetadata(packet)
        )
      }),
      JSON.stringify(
        packets.map((packet) => packet.geometry.debugMeta),
    ...
    ;[
      { name: 'packet', polygons: packetPolygons },
      ...
      expectInsideSolidLowerHighCurvatureCoverageForTest({
        name,
        polygons,
        legalFaceBoundaries,
        strokeWidth
      ...
      if (name !== 'render-clip') {
        expect(
          endpointJoinShape.probeFound,
          JSON.stringify({ name, endpointJoinShape }, null, 2)
        ).toBe(true)
        expect(
          endpointJoinShape.outsideExpectedJoinCoverage,
          JSON.stringify({ name, endpointJoinShape }, null, 2)
        ).toEqual([])
        expect(
          endpointJoinShape.missingExpectedJoinCoverage,
          JSON.stringify({ name, endpointJoinShape }, null, 2)
        ).toEqual([])
      }
    })
  ...
  it('should run: keep self-check self-intersecting solid join matrix near the authored source path', async () => {
    ...
    const outsideRenderMasks: Partial<
      ...
      >
    > = {}
    const insideRenderMasks: Partial<
      Record<
        'miter' | 'round' | 'bevel',
        {
          clipPolygons: Vec2[][]
    ...
    expect(roundMask?.strokePaths.length).toBeGreaterThan(0)
    expect(bevelMask?.strokePaths.length).toBeGreaterThan(0)
    expect(roundMask?.strokeMaskPolygons).toEqual([])
    expect(bevelMask?.strokeMaskPolygons).toEqual([])
    expect(roundMask?.strokePathStyle?.join).toBe('round')
    expect(bevelMask?.strokePathStyle?.join).toBe('bevel')

    const insideMiterMask = insideRenderMasks.miter
    const insideBevelMask = insideRenderMasks.bevel
    const insideRoundMask = insideRenderMasks.round
    const insideCornerProbeCenters =
      getInsideSolidCornerTransitionProbesForTest(legalFaceBoundaries, 10).map(
        (probe) => probe.vertex
      )
    const insideMaskOnlyProbeCenters =
      getInsideSolidMaskOnlyCornerProbesForTest(legalFaceBoundaries, 10)
    const insideMaskOnlyProbePoints = insideMaskOnlyProbeCenters.flatMap(
      (probe) => probe.samplePoints
    )

    expect(insideMiterMask).toBeDefined()
    expect(insideBevelMask).toBeDefined()
    expect(insideRoundMask).toBeDefined()
    ...
    expect(
      insideMaskOnlyProbeCenters.length,
      JSON.stringify({ insideMaskOnlyProbeCenters }, null, 2)
    ).toBeGreaterThanOrEqual(5)
    expect(
      insideMaskOnlyProbePoints.length,
      JSON.stringify({ insideMaskOnlyProbeCenters }, null, 2)
    ).toBeGreaterThanOrEqual(20)
    const insideMiterSourcePolygons = getRenderEntrySourceStrokePolygonsForTest(
      {
        backend,
        entry: insideMiterMask,
        sourcePath,
        join: 'miter',
        strokeWidth: 10
      }
    )
    const insideBevelSourcePolygons = getRenderEntrySourceStrokePolygonsForTest(
      {
        backend,
        entry: insideBevelMask,
        sourcePath,
        join: 'bevel',
        strokeWidth: 10
      }
    )
    const insideRoundSourcePolygons = getRenderEntrySourceStrokePolygonsForTest(
      {
        backend,
        entry: insideRoundMask,
        sourcePath,
        join: 'round',
        strokeWidth: 10
      }
    )
    const insideMiterVisiblePolygons = getRenderEntryVisiblePolygonsForTest({
      backend,
      entry: insideMiterMask,
      sourcePath,
      join: 'miter',
    ...
    const insideBevelVisiblePolygons = getRenderEntryVisiblePolygonsForTest({
      backend,
      entry: insideBevelMask,
      sourcePath,
      join: 'bevel',
    ...
    const insideRoundVisiblePolygons = getRenderEntryVisiblePolygonsForTest({
      backend,
      entry: insideRoundMask,
      sourcePath,
      join: 'round',
    ...
    ;[insideMiterMask, insideBevelMask, insideRoundMask].forEach((mask) => {
      expect(
        mask?.metadata.every((meta) => {
          const typedMeta = meta as typeof meta & {
            solidMaskModelInternalCornerJoinMode?: string
            solidMaskModelAdjacencyProbe?: string[]
          }
          return (
            typedMeta.solidMaskModelInternalCornerJoinMode ===
              'stroke-join-aware-face-corner' &&
            (
              typedMeta as typeof typedMeta & {
                solidMaskModelJoinEligibilityMode?: string
              }
            ).solidMaskModelJoinEligibilityMode === 'internal-face-only' &&
            typedMeta.solidMaskModelAdjacencyProbe?.includes(
              'all-internal-pentagon-corner-join-shapes'
            ) === true &&
            typedMeta.solidMaskModelAdjacencyProbe?.includes(
        ...
        JSON.stringify(mask?.metadata, null, 2)
      ).toBe(true)
    })

    const insideMiterVsBevel = countVisibleMaskCoverageDifferences({
      firstClipPolygons: insideMiterMask?.clipPolygons ?? [],
      firstStrokeMaskPolygons: insideMiterSourcePolygons,
      secondClipPolygons: insideBevelMask?.clipPolygons ?? [],
      secondStrokeMaskPolygons: insideBevelSourcePolygons,
      centers: insideCornerProbeCenters,
      radius: 28,
      step: 1
    })
    const insideMiterVsRound = countVisibleMaskCoverageDifferences({
      firstClipPolygons: insideMiterMask?.clipPolygons ?? [],
      firstStrokeMaskPolygons: insideMiterSourcePolygons,
      secondClipPolygons: insideRoundMask?.clipPolygons ?? [],
      secondStrokeMaskPolygons: insideRoundSourcePolygons,
      centers: insideCornerProbeCenters,
      radius: 28,
      step: 1
    })
    const insideBevelVsRound = countVisibleMaskCoverageDifferences({
      firstClipPolygons: insideBevelMask?.clipPolygons ?? [],
      firstStrokeMaskPolygons: insideBevelSourcePolygons,
      secondClipPolygons: insideRoundMask?.clipPolygons ?? [],
      secondStrokeMaskPolygons: insideRoundSourcePolygons,
      centers: insideCornerProbeCenters,
      radius: 28,
    ...
    const insideMiterVsBevelClipOnly =
      countClipPolygonCoverageDifferencesForTest({
        firstClipPolygons: insideMiterMask?.clipPolygons ?? [],
        secondClipPolygons: insideBevelMask?.clipPolygons ?? [],
        centers: insideCornerProbeCenters,
        radius: 28,
    ...
    const insideMiterVsBevelStrokeOnly =
      countClipPolygonCoverageDifferencesForTest({
        firstClipPolygons: insideMiterSourcePolygons,
        secondClipPolygons: insideBevelSourcePolygons,
        centers: insideCornerProbeCenters,
        radius: 28,
        step: 1
      })
    const maskOnlyMiterVsBevel =
      countClipPolygonCoverageDifferencesAtPointsForTest({
        firstClipPolygons: insideMiterMask?.clipPolygons ?? [],
        secondClipPolygons: insideBevelMask?.clipPolygons ?? [],
        points: insideMaskOnlyProbePoints
      })
    const maskOnlyMiterVsRound =
      countClipPolygonCoverageDifferencesAtPointsForTest({
        firstClipPolygons: insideMiterMask?.clipPolygons ?? [],
        secondClipPolygons: insideRoundMask?.clipPolygons ?? [],
        points: insideMaskOnlyProbePoints
      })
    const maskOnlyBevelVsRound =
      countClipPolygonCoverageDifferencesAtPointsForTest({
        firstClipPolygons: insideBevelMask?.clipPolygons ?? [],
        secondClipPolygons: insideRoundMask?.clipPolygons ?? [],
        points: insideMaskOnlyProbePoints
    ...
    ;[
      ...
      insideMiterVsRound,
      insideBevelVsRound,
      insideMiterVsBevelClipOnly,
      insideMiterVsBevelStrokeOnly
    ].forEach((comparison) => {
      expect(comparison.changed).toBeGreaterThanOrEqual(0)
    })
    expect(
      Math.max(
        maskOnlyMiterVsBevel.changed,
        maskOnlyMiterVsRound.changed,
        maskOnlyBevelVsRound.changed
      ),
      JSON.stringify(
        {
          message:
            'inside solid mask-only triangle/non-pentagon corners must stay join-invariant',
          insideMaskOnlyProbeCenters,
          insideMaskOnlyProbePoints,
    ...
    expectInsideSolidRoundCornerSmoothnessForTest({
      ...
      polygons: insideRoundVisiblePolygons,
      legalFaceBoundaries,
      strokeWidth: 10
    })
    expectInsideSolidRightBottomSourceSegmentAdherenceForTest({
      name: 'inside solid miter visible geometry',
      polygons: insideMiterVisiblePolygons,
      sourcePath,
      strokeWidth: 10,
      targetSegmentEndpoints:
        getSelfCheckRightBottomSourceSegmentEndpointsForTest(),
    ...
    expectInsideSolidRightBottomSourceSegmentAdherenceForTest({
      ...
      polygons: insideBevelVisiblePolygons,
      sourcePath,
      strokeWidth: 10,
      targetSegmentEndpoints:
        getSelfCheckRightBottomSourceSegmentEndpointsForTest(),
    ...
    if (
      (roundMask?.strokePaths.length ?? 0) === 0 ||
      (bevelMask?.strokePaths.length ?? 0) === 0
    ) {
      expect(
