...
import {
  ...
  sliceStrokeIntervalPoints
} from '../components/stroke-render/stroke-interval-frames'

describe('stroke interval frame slicing', () => {
  it('should run: preserve non-uniform width probes through interval slicing on the shared runtime helper', () => {
    const frames = sliceStrokeIntervalFrames(
      [
  ...
  it('should run: preserve asymmetric variable-width probe data through seam-wrap slicing without assuming uniform widths', () => {
    ...
    expect(
      frames.some(
        (frame) => Math.abs(frame.widthLeft - frame.widthRight) > 1e-6
      )
    ).toBe(true)
  })
  ...
  it('should run: slice point-only intervals without manufacturing unused width probes', () => {
    const points = sliceStrokeIntervalPoints(
      [
        { x: 0, y: 0 },
        { x: 50, y: 0 },
        { x: 100, y: 0 }
  ...
  it('should run: reuse precomputed point slicer topology without changing point output', () => {
    const points = [
      { x: 0, y: 0 },
      { x: 50, y: 0 },
      { x: 50, y: 40 },
    ...
    const slicer = createStrokeIntervalPointSlicer(points, true)

    expect(slicer.slice(110, 10, true)).toEqual(
      sliceStrokeIntervalPoints(points, true, 110, 10, true)
    )
  })

  it('should run: reuse precomputed frame slicer topology without changing frame output', () => {
    const frames = [
      { x: 0, y: 0, widthLeft: 2, widthRight: 2 },
      { x: 50, y: 0, widthLeft: 4, widthRight: 4 },
      { x: 50, y: 40, widthLeft: 6, widthRight: 6 },
    ...
    const slicer = createStrokeIntervalFrameSlicer(frames, true)

    expect(slicer.slice(110, 10, true)).toEqual(
      sliceStrokeIntervalFrames(frames, true, 110, 10, true)
    )
  })
})
