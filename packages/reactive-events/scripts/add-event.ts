...
program
  ...
  .action(async (opts) => {
    ...
    const pascalScope = config.scope
      .split('-')
    ...
    const eventNamePascal =
      config.name.charAt(0).toUpperCase() + config.name.slice(1)
    const eventNameCamel =
      config.name.charAt(0).toLowerCase() + config.name.slice(1)
    const eventNameConstant = config.name
    ...
    if (enumDecl.getMember(eventNameConstant)) {
      console.warn(
        chalk.yellow(
          `Event ${eventNameConstant} already exists in ${enumName}. Skipping.`
        )
      )
    } else {
      enumDecl.addMember({
    ...
    if (config.pattern === 'async') {
      ...
      if (enumDecl.getMember(finishConstant)) {
        console.warn(
          chalk.yellow(
            `Event ${finishConstant} already exists in ${enumName}. Skipping.`
          )
        )
      } else {
        enumDecl.addMember({
    ...
    const payloadTypeString = config.payload
    ...
    const mainEventInterface = eventsSourceFile.addInterface({
      ...
      isExported: true,
      properties: [{ name: 'type', type: 'EventTypes' }]
    })
  ...
  requestId: string
    ...
    if (unionTypeAlias) {
      const typeNode = unionTypeAlias.getTypeNode()
      if (typeNode) {
        ...
        // Add Finish
        if (
          config.pattern === 'async' &&
          !currentType.includes(finishEventInterfaceName)
        ) {
      ...
      console.warn(
        chalk.yellow(
          `Union type ${unionName} not found. Skipped updating union.`
        )
      )
    }
    ...
    const subscribesPath = path.resolve(
      __dirname,
      `../src/${config.scope}/subscribes.ts`
    )
    const subscribesSourceFile =
      project.addSourceFileAtPathIfExists(subscribesPath)
    ...
    // Add Imports
    const subEventsImport = subscribesSourceFile.getImportDeclaration(
      (d) => d.getModuleSpecifierValue() === './events'
    )
    if (subEventsImport) {
    ...
    const pubEventsImport = publishSourceFile.getImportDeclaration(
      (d) => d.getModuleSpecifierValue() === './events'
    )
    if (pubEventsImport) {
    ...
    if (config.pattern === 'async') {
      ...
      if (utilsImport) {
        if (
          !utilsImport
            .getNamedImports()
            .some((n) => n.getName() === 'generateRequestId')
        ) {
      ...
      const rxjsImport = publishSourceFile.getImportDeclaration(
      ...
      // Import subscriber
      const pubSubsImport = publishSourceFile.getImportDeclaration(
        (d) => d.getModuleSpecifierValue() === './subscribes'
      )
      if (pubSubsImport) {
    ...
    const hasPayload =
      (config.payload && config.payload !== '{}') || config.pattern === 'async'
    ...
    const optionsArg = config.options
      ? `, options?: ${eventInterfaceName}['options']`
      : ''
