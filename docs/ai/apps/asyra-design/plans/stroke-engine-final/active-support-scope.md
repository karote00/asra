...
| Self-intersecting closed paths | `supported now` for constrained dashed local-side approximation visibility; `implementation in progress` for constrained solid exact arranged visual collapse; constrained dashed exact face promotion gated | Dashed constrained packets preserve the authored `inside/outside` side so supported dashed strokes do not disappear. Solid constrained candidates are defined to use typed one-sided per-source-span candidates, same-visual overlap collapse, and a shared final face source, but product support is not claimed until vector-6 global/local visual gates pass. Source self-intersections are not product clipping boundaries by themselves: the reported vector-6 star must keep visible authored crossing strokes, and only adjacent authored-segment side constraints may clip endpoint/local overhang. Constrained dashed packets remain local-side even when an exact backend is selected because dashed interval ownership and legal-domain clipping can still remove valid internal dash regions. 2026-04-29 Figma outline exports confirm inside and outside dashed self-intersections produce different filled-component structures, so dashed exact support still classifies each side separately before promotion can be claimed. |
