...
- the first join/cap round representatives
- the first gradient paint gradient-paint representative

It answers:

> When a shape-generated or vector-generated closed path uses a
> constrained dashed stroke whose visible interval either covers the full
> closed loop, stays within one legal edge span, or crosses one legal
> corner, does the real app/runtime path render the rule-driven slices while
> keeping diagnostic-only round/paint evidence out of the product oracle?
...
- one single-edge visible interval on a closed path
- shape-generated `rect`
- corner-spanning rule representatives on a closed path:
...
- rectangle outside constrained dashed full-loop stroke renders a visible outer
  band on the same stroke pipeline while the rectangle center remains unfilled
...
- the first vector-generated closed rectangle-equivalent single-edge fixture
  keeps the same inside/outside interval-local semantics on the formal
  stroke pipeline
...
- the first corner-spanning constrained dashed representatives keep one
  ...
  remains absent for the current rule representatives:
  - `inside + bevel`
  - `inside + miter`
  - `outside + bevel`
  - `outside + miter`
- the first vector-generated closed rectangle-equivalent corner-spanning
  ...
  coverage on the shared stroke pipeline for:
  - `inside + bevel`
  - `inside + miter`
  - `outside + bevel`
  - `outside + miter`
- the first broader vector-generated closed non-rectangle-equivalent
  ...
  interval across the formal top-right legal corner for:
  - `inside + bevel`
  - `inside + miter`
- the next broader vector-generated closed non-rectangle-equivalent
  ...
  interval across the same formal top-right legal corner for:
  - `outside + bevel`
  - `outside + miter`
- oval inside constrained dashed full-loop stroke renders a visible inner band
  on the same stroke pipeline without filling the oval center
...
- the first vector-generated closed rectangle-equivalent fixture keeps the same
  inside/outside visible-band semantics on the shared stroke pipeline
...
- the first vector-generated closed rectangle-equivalent round-join fixture
  ...
  semantics on the shared stroke pipeline
...
- shape-generated and vector-generated rectangle-equivalent fixtures keep
  ...
  the next join/cap source-equivalence rule equivalence gate
...
- the next broader vector-generated closed non-rectangle-equivalent round-cap
  ...
  dashed semantics on the shared stroke pipeline
...
join` constrained dashed coverage on the uniform-width corner-spanning topology family stroke pipeline
...
outside + round join` constrained dashed coverage on the uniform-width Family
  C stroke pipeline
- formal full-loop constrained dashed round joins use the shared constrained
  round geometry path; they must not substitute miter geometry as a proxy
- the first gradient paint gradient-paint representative keeps the same constrained
...
- the next gradient paint gradient-paint representative keeps the same constrained
  dashed `rect + full-loop + outside` geometry while swapping only paint to a
  local-bounds linear gradient on the bounded app/runtime path
- the next gradient paint vector-generated gradient-paint representative keeps the
...
- the next gradient paint vector-generated gradient-paint representative keeps the
  ...
  rectangle-equivalent `vector` path while swapping only paint to the same
  local-bounds linear gradient field
- the next broader gradient paint vector-generated gradient-paint representative keeps
...
- shape-generated and vector-generated rectangle-equivalent round-join
  full-loop coverage stays within the declared tolerance on the first join/cap
  source-equivalence rule equivalence gate
- shape-generated and vector-generated rectangle-equivalent round-cap
  single-edge coverage stays within the declared tolerance on the next join/cap
  source-equivalence rule equivalence gate
- shape-generated and vector-generated rectangle-equivalent full-loop gradient
  coverage stays within the declared tolerance on the first gradient paint source-equivalence rule
  equivalence gate
- the first broader vector-generated closed non-rectangle-equivalent fixture
  keeps the same inside/outside visible-band semantics on the formal product
  path
- shape-generated and vector-generated rectangle-equivalent full-loop coverage
  stays within the declared tolerance on the first source-equivalence rule equivalence gate

## Additional formal product behavior
...
- open-path constrained dashed `vector` paths keep authored `inside` /
  ...
  geometry for formal simple open paths
- real-created open single-network `vector` paths with repeated dash intervals
  ...
  `center`, and render through exact constrained open-path semantics through the
  formal domain plan
...
- corner-spanning constrained dashed slices outside this benchmark are covered by
  separate formal product suites, except for:
...
- broader non-full-loop constrained dashed slices beyond the first closed
  ...
  covered by separate formal product suites
- multi-network vector constrained dashed slices outside this benchmark are
  covered by separate formal product suites
- constrained dashed gradient-paint cases beyond the first gradient paint
  representative are covered by separate formal product suites:
  - only these gradient paint representatives are covered here:
  ...
  - broader gradient-paint slices beyond these first vector-generated
    representatives are covered by separate formal product suites
  - corner-spanning constrained dashed gradient paint remains covered by separate formal product suites
    beyond the first formal representative:
    - shape-generated `rect + inside + bevel + corner-spanning + local-bounds linear gradient paint`
    - shape-generated `rect + outside + bevel + corner-spanning + local-bounds linear gradient paint`
    - closed rectangle-equivalent `vector + inside + bevel + corner-spanning + local-bounds linear gradient paint`
    - closed non-rectangle-equivalent quadrilateral `vector + inside + bevel + corner-spanning + local-bounds linear gradient paint`
  - variable-width plus gradient combined slices are covered by separate formal product suites
...
- repeat with `position: outside` and measure formal outer-band coverage
...
- repeat on one rectangle with `outside + single-edge + round cap` and verify:
  - constrained dashed coverage exists on the formal exterior terminal cap
  - constrained dashed coverage exists on the formal exterior body span
...
- repeat on one vector path patched to a closed rectangle-equivalent network
  ...
  - body coverage exists on the formal interval
  - exterior leakage remains absent
...
- repeat on the reported closed star-like single-network vector with repeated
  ...
  - the constrained switch uses the formal constrained dashed packet path
...
- repeat on one rectangle with a corner-spanning visible interval and verify:
  - coverage exists on both sides of the formal inside corner
  ...
  - repeat once with `position: outside + join: bevel` and verify:
    - coverage exists on both formal exterior corner-adjacent spans
    - unrelated top-edge spans remain absent
    - interior leakage remains absent
    - the center remains absent
...
- create one vector path, patch it to a closed rectangle-equivalent network,
  and repeat the same inside/outside probes on the formal vector fixture
- repeat on one vector path patched to a closed rectangle-equivalent network
...
- repeat on one vector path patched to a closed non-rectangle-equivalent
  ...
  - coverage exists on the formal top edge near the corner
  ...
  - repeat once with `position: outside + join: bevel`
    - coverage exists on the formal top outside span near the corner
...
- formal inner-band coverage must be `> 0.55`
- formal exterior leakage must be `< 0.12`
- formal center leakage must be `< 0.03`
- formal outer-band coverage must be `> 0.55`
- formal single-edge interval coverage must be `> 0.55`
- vector single-edge round-cap terminal coverage may use a tighter probe with a
  ...
  representative
- formal corner-spanning edge coverage must be `> 0.55`
- non-product same-edge gap coverage must be `< 0.03`
- formal interior leakage for the outside slice must be `< 0.12`
...
- shape/vector single-edge formal-interval coverage delta must be `< 0.08`
