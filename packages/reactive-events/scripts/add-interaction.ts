...
program
  ...
  .action(async () => {
    ...
    const scopeDirInteraction = path.join(
...
async function updateRegistry(
  ...
  if (!variableDecl) {
    logWarning(
      'Could not find InteractionActions constant. Skipping registry update.'
    )
    return
  }

  // Find the object literal initializer
  const objectLiteral = variableDecl?.getInitializerIfKind(
...
async function updateReactiveEvents(
  ...
  const eventsFile = project.addSourceFileAtPath(
    path.join(eventsDir, 'events.ts')
  )
  ...
  // 2. publish.ts
  const publishFile = project.addSourceFileAtPath(
    path.join(eventsDir, 'publish.ts')
  )
  ...
  // 3. subscribes.ts
  const subscribesFile = project.addSourceFileAtPath(
    path.join(eventsDir, 'subscribes.ts')
  )
  ...
  const typesFile = project.addSourceFileAtPath(
    path.join(REACTIVE_EVENTS_PKG, 'src', 'types.ts')
  )
...
export const decideFrom${pascalName}Rules = (
  ...
  */
  ...
  // Update Rules index
  const rulesIndex = project.addSourceFileAtPath(
    path.join(scopeDirRules, 'index.ts')
  )
...
export const decide${pascalName}Behavior = (
  ...
  const behaviorIndex = project.addSourceFileAtPath(
    path.join(scopeDirBehavior, 'index.ts')
  )
  behaviorIndex.addExportDeclaration({
    moduleSpecifier: `./${kebabName}-behavior`
  })
  ...
  if (fs.existsSync(handlerPath)) {
    ...
    const handlerObj = handlerFile
      .getVariableDeclaration(handlerVarName)
...
export const ${handlerVarName} = {
  [InteractionActions.${constantName}]: (
    ...
    const handlersIndex = project.addSourceFileAtPath(
      path.join(scopeDirHandlers, 'index.ts')
    )
    ...
    const exportVar = handlersIndex.getVariableDeclaration(
      'InteractionCoreHandlers'
    )
    const exportObj = exportVar?.getInitializerIfKind(
      SyntaxKind.ObjectLiteralExpression
    )
    if (exportObj) {
