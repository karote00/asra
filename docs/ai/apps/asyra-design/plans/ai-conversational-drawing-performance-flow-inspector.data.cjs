  ...
  const steps = [
    ...
    },
    {
      ...
      conditions: [
        ...
        'The exact guarded 7,076-element creation-only endpoint is a high-performance test with a 400-percent raw same-snapshot complete client-browser limit; guarded 16-item and 1,280-item safety or attribution cases retain the 250-percent frontend limit, and every case retains the 400-percent raw same-snapshot aggregate frontend, backend, and harness hard safety limit.',
        ...
        'One complete operating-system ps snapshot with the exact required PID set supplies the raw percent-CPU values for every tracked process. Polling nominally every 250 milliseconds only asks the system for another raw value; cadence is not a measurement window and never participates in a CPU-percentage formula.',
        'Periodic and phase-boundary sampling share one serialized OS sample and state-consumption queue, so no overlapping ps command or out-of-order state update can combine raw values from different snapshots.',
        'A fixed 375-millisecond observation-gap ceiling fails closed because the guard may have missed a raw system peak; it never constructs a longer interval average.',
        'The proof-class raw same-snapshot complete client-browser limit—400 percent for the exact 7,076-element endpoint and 250 percent for 16-item or 1,280-item safety or attribution—plus the 400-percent aggregate limit applies before and after guard readiness; bootstrap overload is reported as bootstrap rather than attributed to a product request.',
        'The browser role reports root-browser, GPU, utility, other browser subprocesses, and each renderer PID separately while retaining every raw same-snapshot system percent in the active proof-class frontend total and the fixed 400-percent aggregate safety total. Only the highest complete raw client-browser snapshot is retained as the performance peak; backend and harness CPU never enter that peak, although their raw values remain inside aggregate stop evaluation and a 400-percent violation report. Page-target CDP reports TaskDuration, ScriptDuration, LayoutDuration, and RecalcStyleDuration, CDP-visible worker targets are listed as visible worker target evidence, and any renderer CPU not explained by those signals remains explicitly residual renderer evidence rather than being guessed as page or worker ownership.',
        ...
        'Every phase-boundary sample passes through the same active proof-class raw same-snapshot frontend evaluation—400 percent for the exact 7,076-element endpoint or 250 percent for 16-item and 1,280-item safety or attribution—and the fixed 400-percent aggregate safety evaluation as the periodic sampler and requires exact PID set equality; any observed process identity change before an accepted terminal heartbeat makes attribution invalid, and raw OS CPU can never be the sole owner-attribution signal.',
        ...
        'Playwright progress observation performs at most one O(1) scalar sample in each Actor every five seconds during the product window; the independent 250-millisecond raw operating-system sampler, ten-second heartbeat deadline, and twenty-second progress deadline remain unchanged.',
        ...
        'The 250-millisecond polling cadence is armed before the immediate first raw system snapshot, every sample has a 200-millisecond hard timeout, and guard SIGINT, SIGTERM, SIGHUP, exceptional exit, sampling failure, or benchmark failure terminates only the fixed registered process groups.',
        ...
        'A single raw operating-system snapshot whose complete client-browser sum is above the active proof-class limit—400 percent for the exact 7,076-element endpoint or 250 percent for 16-item and 1,280-item safety or attribution—or whose aggregate frontend/backend/harness sum is above 400 percent stops the benchmark immediately and marks the active architecture attempt invalid; configuration cannot relax either proof-class limit.',
        ...
        'The hard CRDT product-flow deadline from Actor A request submission through Actor B convergence is 180 seconds; the guarded Playwright ceiling is 240 seconds so bounded bootstrap, final assertions, and teardown cannot preempt that product deadline.',
        'Crossing either raw CPU limit, the 180-second product-flow deadline, or the 240-second guarded Playwright ceiling terminates the current benchmark action and its exact tracked processes but never terminates the implementation task. The same owner immediately captures the first blocker, performs bounded root-cause analysis, re-reads this Inspector contract, revises only its owner plan and formal oracle, and executes the resulting new iteration before any downstream owner may advance.',
        ...
        'The raw 251.7-percent frontend and 259.0-percent aggregate local-source observation is below both revised 7,076-element limits, so it is not an accepted stop or endpoint proof; after focused threshold gates, the local-source guarded 7,076-element proof runs once under the corrected 400-percent frontend and aggregate limits before remote apply advances.',
  ...
  const acceptanceContracts = [
    ...
    },
    {
      ...
      assertions: [
        ...
        'The process-tree guard terminates tracked test work when a raw same-snapshot complete client-browser system value exceeds the active proof-class limit—400 percent for the exact 7,076-element endpoint or 250 percent for 16-item and 1,280-item safety or attribution—a raw same-snapshot aggregate frontend/backend/harness system value exceeds 400 percent, the heartbeat becomes stale, or progress stalls; it reports the last phase plus Actor A/B element counts, and only a raw CPU-limit stop invalidates that architecture attempt.',
