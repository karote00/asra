...
test.describe('new stroke visual/E2E coverage map', () => {
  ...
  test('references existing spec anchors, inspector runtime routes, and post-runtime validation gates', () => {
    ...
    expect(readme).toContain('Post-runtime validation')
    expect(readme).toContain('Only after an explicit user request')
    expect(activePlan).toContain('explicit-request-only validation methods')
