...
| --- | --- | --- | --- |
| Self-intersecting closed `inside/outside` dashed exact stroke | `defined as Figma-like even-odd boundary-contour stroke` | `implemented for the current boundary-contour product path` | Self-intersecting constrained dashed product geometry is generated from the shared resolved geometry model's even-odd legal-region boundary contours, including hole boundaries such as the center pentagon in a self-intersecting star. Fill consumes legal regions; inside/outside stroke consumes legal/exterior or legal/hole boundary contours; center stroke is excluded. The earlier authored-side local-side visibility path is no longer the product contract and cannot be cited as supported behavior. |
| Self-intersecting closed `inside/outside` solid stroke | `defined for current product slice` | `implementation in progress; not yet active support` | Solid constrained full-loop candidates must be generated as typed one-sided per-source-span candidates, promoted through the arrangement path, and collapsed to one same-visual layer before product render / hit / export may claim support. Source self-intersections are not clipping boundaries by themselves: the reported vector-6 star must keep all five authored segments visible. This slice is not considered supported until the global visual, five endpoint crops, five self-intersection crops, single-layer opacity oracle, and dashed regression gates all pass. |
...
| Closed constrained dashed non-full-loop | `supported now` for rectangle-equivalent, broader-simple-closed, sampled-simple-closed, seam-wrapping, and non-self-intersecting visible local-side slices | Supported slices build interval-local one-sided geometry. Each visible interval emits one packet; that packet may contain multiple bounded segment-cell polygons when a merged ribbon would self-overlap or fan. Sampled-simple constrained dashed sources preserve `inside/outside` and emit explicitly marked `resolutionStatus: "local-side-approximation"` packets when the current geometry is local-side. Self-intersecting closed paths are not part of this support row anymore; they are governed by the Figma-like even-odd boundary-contour row. A selected backend must not replace local-side packets with exact arrangement faces unless the exact path proves segment-local clipping parity. Center fallback is forbidden. Seam-wrapping intervals are sliced across the seam instead of being dropped. |
...
| Self-intersecting closed paths | `supported now` for Figma-like even-odd boundary-contour constrained dashed; `implementation in progress` for constrained solid exact arranged visual collapse | Dashed constrained `inside/outside` is generated from even-odd legal-region boundary contours. Outer boundaries and hole boundaries are both dash domains; inside emits toward the legal face side and outside emits toward the opposite side. Each split contour edge is an independent dash domain with half-dash endpoint placement and interior dash/gap distribution. Center stroke does not use these side rules. The previous authored-side local-side approximation is demoted from active support and must not be used as product render evidence. |
