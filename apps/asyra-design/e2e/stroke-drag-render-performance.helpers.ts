...
const strokeCaseMatchesScenarioFilter = (strokeCase: StrokeCase) =>
  getDragTargetsForStrokeCase(strokeCase).some((target) =>
    matchesScenarioFilter(`${strokeCase.label}:${target}`)
  )

const burstStrokeCaseMatchesScenarioFilter = (strokeCase: StrokeCase) => {
  const target: DragTarget =
    strokeCase.pathKind === 'open-self-intersecting'
      ? 'first-anchor-in-control'
      : 'out-control'
  return matchesScenarioFilter(`${strokeCase.label}:${target}`)
}
...
const measureMoveVectorElement = async (
  ...
  for (let step = 1; step <= DRAG_STEP_COUNT; step += 1) {
    ...
    const dragFrameId = `${MOVE_VECTOR_SCENARIO_LABEL}:${step}`
    await installFrameProfiler(page, dragFrameId)
    const browserStart = await page.evaluate(() => performance.now())
  ...
  return {
    label: MOVE_VECTOR_SCENARIO_LABEL,
...
export interface StrokeDragRenderPerformanceTestCase {
  title: string
  strokeCases: StrokeCase[]
  burstStrokeCases: StrokeCase[]
  includeMoveVector: boolean
...
export const createStrokeDragRenderPerformanceUXGateCases = ({
  suiteLabel,
  strokeCases,
  burstStrokeCases = [],
  includeMoveVector = false
}: StrokeDragRenderPerformanceSuiteOptions): StrokeDragRenderPerformanceTestCase[] => {
  const testCases: StrokeDragRenderPerformanceTestCase[] = strokeCases
    .filter(strokeCaseMatchesScenarioFilter)
    .map((strokeCase) => ({
      title: `measures ${suiteLabel} ${strokeCase.label} real browser drag rendering with complete stroke render probes`,
      strokeCases: [strokeCase],
      burstStrokeCases: [],
      includeMoveVector: false
    }))

  if (includeMoveVector && matchesScenarioFilter(MOVE_VECTOR_SCENARIO_LABEL)) {
    testCases.push({
      title: `measures ${suiteLabel} move-vector real browser drag rendering with complete stroke render probes`,
      strokeCases: [],
      burstStrokeCases: [],
  ...
  testCases.push(
    ...burstStrokeCases
      .filter(burstStrokeCaseMatchesScenarioFilter)
      .map((burstStrokeCase) => ({
        title: `measures ${suiteLabel} ${burstStrokeCase.label} burst coalescing with complete stroke render probes`,
        strokeCases: [],
        burstStrokeCases: [burstStrokeCase],
        includeMoveVector: false
      }))
  )
...
export const runStrokeDragRenderPerformanceUXGateCase = async (
  ...
  const { strokeCases, burstStrokeCases, includeMoveVector } = testCase
      ...
      `STROKE_DRAG_E2E_BURST_METRICS ${JSON.stringify({
        ...
        burstMetrics: burstMetrics.map((burstMetric) => ({
          ...
          frameCoordinatorCounters: {
            ...
            dirtyElementCount: burstMetric.counters['dirty-element-count'] ?? 0,
          ...
          selectedPhaseTotals: {
            'feature:event:input.drag.update':
              burstMetric.phaseTotalMs['feature:event:input.drag.update'] ?? 0,
  ...
  console.log(
    `STROKE_DRAG_E2E_PHASE_METRICS ${JSON.stringify({
      ...
      scenarios: metrics.map((metric) => ({
        ...
        paintSchedulingP95MinusBaseline: metric.paintSchedulingP95MinusBaseline,
        instrumentationGaps: metric.instrumentationGaps,
        instrumentationErrors: metric.instrumentationErrors,
