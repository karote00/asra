  ...
  const steps = [
    ...
    },
    {
      ...
      conditions: [
        ...
        'The exact guarded 7,076-element creation-only endpoint is a high-performance test with a 500-percent raw same-snapshot complete client-browser limit and a 500-percent raw same-snapshot aggregate frontend, backend, and harness limit; guarded 16-item and 1,280-item safety or attribution cases retain the 250-percent frontend limit and the 400-percent aggregate limit.',
        ...
        'The proof-class raw same-snapshot complete client-browser limit—500 percent for the exact 7,076-element endpoint and 250 percent for 16-item or 1,280-item safety or attribution—plus the proof-class aggregate limit—500 percent for the exact 7,076-element endpoint and 400 percent for 16-item or 1,280-item safety or attribution—applies before and after guard readiness; bootstrap overload is reported as bootstrap rather than attributed to a product request.',
        'The browser role reports root-browser, GPU, utility, other browser subprocesses, and each renderer PID separately while retaining every raw same-snapshot system percent in the active proof-class frontend and aggregate safety totals. Only the highest complete raw client-browser snapshot is retained as the performance peak; backend and harness CPU never enter that peak, although their raw values remain inside aggregate stop evaluation and the proof-class violation report. Page-target CDP reports TaskDuration, ScriptDuration, LayoutDuration, and RecalcStyleDuration, CDP-visible worker targets are listed as visible worker target evidence, and any renderer CPU not explained by those signals remains explicitly residual renderer evidence rather than being guessed as page or worker ownership.',
        ...
        'A single raw operating-system snapshot whose complete client-browser sum is above the active proof-class limit—500 percent for the exact 7,076-element endpoint or 250 percent for 16-item and 1,280-item safety or attribution—or whose aggregate frontend/backend/harness sum is above the active aggregate proof-class limit—500 percent for the exact 7,076-element endpoint or 400 percent for 16-item and 1,280-item safety or attribution—stops the benchmark immediately and marks the active architecture attempt invalid; configuration cannot relax either proof-class limit.',
  ...
  const acceptanceContracts = [
    ...
    },
    {
      ...
      assertions: [
        ...
        'The process-tree guard terminates tracked test work when a raw same-snapshot complete client-browser system value exceeds the active proof-class limit—500 percent for the exact 7,076-element endpoint or 250 percent for 16-item and 1,280-item safety or attribution—or a raw same-snapshot aggregate frontend/backend/harness system value exceeds its proof-class limit—500 percent for the exact 7,076-element endpoint or 400 percent for 16-item and 1,280-item safety or attribution—the heartbeat becomes stale, or progress stalls; it reports the last phase plus Actor A/B element counts, and only a raw CPU-limit stop invalidates that architecture attempt.',
