...
test('Phase 0 authorities and catalog owners exist', () => {
  ...
  assert.equal(fs.existsSync(inspectorPath), true, 'workspace Inspector must exist')
  assert.equal(fs.existsSync(catalogPath), true, 'catalog owner must exist')
  assert.equal(fs.existsSync(generatorPath), true, 'catalog generator must exist')
  assert.equal(fs.existsSync(bundlePath), true, 'generated browser snapshot must exist')
})
...
test('generated catalog classifies every fixed-root candidate exactly once', () => {
  ...
  assert.equal(new Set(catalog.exclusions.map((item) => item.path)).size, catalog.exclusions.length)
  assert.equal(
