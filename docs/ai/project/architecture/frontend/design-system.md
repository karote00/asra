# Architecture: @asra/design-system

## Core Responsibility

The `@asra/design-system` package serves as the **Component Library** for the application. Its primary responsibility is to encapsulated visual styles and UI behaviors into reusable React components.
...
3.  **Encapsulation**: Components encapsulate their own structure and styling. Consumers (like `apps/ui`) should not need to override styles heavily.
...
-   **Usage**: Components are imported directly into `apps/ui` or other frontend packages that require UI elements.
