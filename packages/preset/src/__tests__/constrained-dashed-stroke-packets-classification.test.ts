...
describe('constrained dashed stroke packets: classification ownership and metadata', () => {
  ...
  it('should classify blocked closed constrained dashed runtime status as blocked without substitute geometry', () => {
    const points = [
      { x: 0, y: 0 },
      { x: 80, y: 0 },
      { x: 80, y: 40 },
  ...
  it('should run: derive render, hit, and export packets from the same constrained dashed full-loop geometry source', () => {
    ...
    const diagnosticHitPackets = withStrokeDiagnosticsMode('full', () =>
      buildSolidCenterStrokeHitTestPackets(acceptedPackets)
    )
    const diagnosticExportPackets = withStrokeDiagnosticsMode('full', () =>
      buildSolidCenterStrokeExportPackets(acceptedPackets)
    )
    ...
    acceptedPackets.forEach((resolved, index) => {
      ...
      const diagnosticHit = diagnosticHitPackets[index]
      const diagnosticExport = diagnosticExportPackets[index]
    ...
    expect(diagnosticExportPackets[0]?.debugMeta).toMatchObject({
