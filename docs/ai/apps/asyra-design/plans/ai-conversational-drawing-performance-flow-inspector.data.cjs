  ...
  const steps = [
    ...
    },
    {
      ...
      conditions: [
        ...
        'Every phase-boundary sample passes through the same fixed raw same-snapshot 250-percent frontend and 400-percent aggregate safety evaluations as the periodic sampler and requires exact PID set equality; any observed process identity change across samples makes attribution invalid, and raw OS CPU can never be the sole owner-attribution signal.',
        ...
        'A single raw operating-system snapshot whose complete client-browser sum is above 250 percent or whose aggregate frontend/backend/harness sum is above 400 percent stops the benchmark immediately and marks the active architecture attempt invalid; configuration cannot relax either limit.',
