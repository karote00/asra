...
describe('constrained solid stroke packets: right-upper internal corner diagnostics', () => {
  it('should run: reject right-upper internal pentagon corner artifact vertices', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    const cases = ['round', 'bevel', 'miter'] as const

    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-upper-root-cause`,
      ...
      const centerStrokePolygons = buildSolidCenterStrokePolygons(
      ...
      const centerStrokeRegions = backend.union(
        centerStrokePolygons.map((polygon) => ({ polygons: [polygon] })),
        'nonzero'
      )
      const segmentedSourceStrokePolygons =
        flattenTestRegionPolygons(centerStrokeRegions)
      const offsetSourceStrokePolygons = buildOffsetSourceStrokePolygonsForTest(
        backend,
        sourcePath,
        joinType,
        10
      )
      const offsetSourceStrokeRegions = backend.union(
        offsetSourceStrokePolygons.map((polygon) => ({ polygons: [polygon] })),
        'nonzero'
      )
      const visiblePolygons =
        entry?.clipPolygons && entry.clipPolygons.length > 0
          ? flattenTestRegionPolygons(
              backend.intersection(
                centerStrokeRegions,
                entry.clipPolygons.map((polygon) => ({ polygons: [polygon] })),
                'nonzero'
              )
            )
          : []
      const offsetVisiblePolygons =
        entry?.clipPolygons && entry.clipPolygons.length > 0
          ? flattenTestRegionPolygons(
              backend.intersection(
                offsetSourceStrokeRegions,
                entry.clipPolygons.map((polygon) => ({ polygons: [polygon] })),
                'nonzero'
              )
            )
          : []
      const descriptorSourceStrokePolygons =
        getRenderEntrySourceStrokePolygonsForTest({
          backend,
          entry,
          sourcePath,
          join: joinType,
      ...
      const descriptorSourceStrokeRegions = backend.union(
        descriptorSourceStrokePolygons.map((polygon) => ({
          polygons: [polygon]
        })),
        'nonzero'
      )
      const descriptorVisiblePolygons =
        entry?.clipPolygons && entry.clipPolygons.length > 0
          ? flattenTestRegionPolygons(
              backend.intersection(
                descriptorSourceStrokeRegions,
                entry.clipPolygons.map((polygon) => ({ polygons: [polygon] })),
                'nonzero'
              )
            )
          : []
      const rightUpperProbe = getRightUpperInsidePentagonCornerProbeForTest(
        legalFaceBoundaries,
        10
      )
      expect(rightUpperProbe).toBeDefined()
      const rightUpperLayerDiagnostics = rightUpperProbe
        ? {
            visible: getInsideSolidCornerSingleLobeAnalysisForTest({
              probe: rightUpperProbe,
              strokeWidth: 10,
              containsPoint: (point) =>
                polygonListContainsPointWithWinding(
                  descriptorVisiblePolygons,
                  point
                )
            }),
            clip: getInsideSolidCornerSingleLobeAnalysisForTest({
              probe: rightUpperProbe,
              strokeWidth: 10,
              containsPoint: (point) =>
                polygonListContainsPointWithWinding(
            ...
            sourceMask: getInsideSolidCornerSingleLobeAnalysisForTest({
              probe: rightUpperProbe,
              strokeWidth: 10,
              containsPoint: (point) =>
                polygonListContainsPointWithWinding(
                  descriptorSourceStrokePolygons,
                  point
                )
            })
          }
      ...
      expect(
        [
          ...(rightUpperLayerDiagnostics?.visible.splitLobeLines ?? []),
          ...(rightUpperLayerDiagnostics?.visible.splitRays ?? []),
          ...(rightUpperLayerDiagnostics?.visible.lobeValleys ?? [])
        ],
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon lobe root cause`,
            reason:
              'visible right-upper corner must be one envelope; layer diagnostics identify whether the split comes from clip or source mask',
            rightUpperLayerDiagnostics
          },
          null,
          2
        )
      ).toEqual([])
      expectNoInsidePentagonCornerArtifactVerticesForTest({
        name: `inside solid ${joinType} right-upper internal pentagon visible geometry`,
        polygons: descriptorVisiblePolygons,
        legalFaceBoundaries,
        strokeWidth: 10
      })
      expectRightUpperInsidePentagonClipContainsSourceJoinForTest({
        name: `inside solid ${joinType} right-upper internal pentagon render clip source-join containment`,
        sourceStrokePolygons: descriptorSourceStrokePolygons,
        clipPolygons: entry?.clipPolygons ?? [],
        legalFaceBoundaries,
        strokeWidth: 10
      })
      const segmentedSourceRootCause =
        getInsidePentagonCornerSplitIntervalDiagnosticsForTest({
          visiblePolygons,
          clipPolygons: entry?.clipPolygons ?? [],
          sourceStrokePolygons: segmentedSourceStrokePolygons,
          legalFaceBoundaries,
          strokeWidth: 10
        }).splitVisible
      expect(
        segmentedSourceRootCause.length,
        JSON.stringify(
          {
            name: `inside solid ${joinType} segmented center-stroke root-cause control`,
            reason:
              'the removed segmented source-stroke mask splits near the internal pentagon corner',
            segmentedSourceRootCause
          },
          null,
          2
        )
      ...
      expectNoInsidePentagonCornerSplitIntervalsForTest({
        name: `inside solid ${joinType} right-upper internal pentagon exact-offset source mask control`,
        visiblePolygons: offsetVisiblePolygons,
        clipPolygons: entry?.clipPolygons ?? [],
        sourceStrokePolygons: offsetSourceStrokePolygons,
        legalFaceBoundaries,
        strokeWidth: 10
      })
      expectNoInsidePentagonCornerSplitIntervalsForTest({
        name: `inside solid ${joinType} right-upper internal pentagon root cause`,
        visiblePolygons: descriptorVisiblePolygons,
        clipPolygons: entry?.clipPolygons ?? [],
        sourceStrokePolygons: descriptorSourceStrokePolygons,
        legalFaceBoundaries,
        strokeWidth: 10
  ...
  it('should run: diagnose right-upper internal pentagon corner micro protrusions after full flow', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    const backend = createClipper2GeometryBackend(await loadClipperModule())
    const cases = ['round', 'bevel'] as const

    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-upper-micro-root-cause`,
        topology.normalizedPoints,
        true,
        [
          createDefaultStroke({
      ...
      const clipPolygons = entry?.clipPolygons ?? []
      const sourceMaskPolygons = getRenderEntrySourceStrokePolygonsForTest({
        backend,
        entry,
        sourcePath,
        join: joinType,
      ...
      expect(packets.length).toBeGreaterThan(0)
      expect(sourceMaskPolygons.length).toBeGreaterThan(0)
      expect(clipPolygons.length).toBeGreaterThan(0)
      expectRenderEntryHasSourceStrokeMaskForTest(entry)
      expectRightUpperInsidePentagonMicroProfileForTest({
        name: `inside solid ${joinType} right-upper internal pentagon micro profile`,
        legalFaceBoundaries,
        strokeWidth: 10,
        layers: [
          {
  ...
  it('should run: locate right-upper internal pentagon micro side-peak owner', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = withStrokeDiagnosticsMode('full', () =>
        buildConstrainedSolidStrokeResolvedPackets(
          `self-check-star-solid-domain:inside:${joinType}:right-upper-micro-side-peak-owner`,
          topology.normalizedPoints,
          true,
          [
            createDefaultStroke({
      ...
      const contributors =
        getRightUpperInsidePentagonMicroPeakContributorsForTest({
          legalFaceBoundaries,
          strokeWidth: 10,
          layers: [
            {
            ...
            {
              name: 'strokeMaskPolygons',
              polygons: strokeMaskPolygons
            },
            {
              name: 'strokePathPolygons',
              polygons: strokePathPolygons
            }
          ],
          faceOwnershipTrace:
            packets[0]?.geometry.debugMeta?.solidMaskModelFaceOwnershipTrace
        })
      const productContributors = contributors.filter(
        (contributor) =>
          contributor.layer !== 'strokePathPolygons' &&
          contributor.layer !== 'renderClip'
      )
      ...
      expect(
        ...
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon micro side-peak owner`,
            reason:
              'right-upper internal pentagon corner must not expose product-visible micro side peaks in the 2000% failure sector; renderClip and sourceStroke rows are diagnostic controls only',
            contributors
          },
          null,
          2
  ...
  it('should run: locate right-upper internal pentagon 2000pct double-tip source', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = withStrokeDiagnosticsMode('full', () =>
        buildConstrainedSolidStrokeResolvedPackets(
          `self-check-star-solid-domain:inside:${joinType}:right-upper-2000pct-double-tip-source`,
          topology.normalizedPoints,
          true,
          [
            createDefaultStroke({
      ...
      const contributors =
        getRightUpperInsidePentagonWideSectorTipContributorsForTest({
          legalFaceBoundaries,
          strokeWidth: 10,
          layers: [
            {
      ...
      expect(
        ...
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon 2000pct double-tip source`,
            reason:
              'right-upper internal pentagon corner must not expose accepted visible/stroke-mask vertices in the wide side-tip sector; renderClip and strokePath rows are diagnostic controls only',
            contributors
          },
          null,
          2
  ...
  it('should run: locate right-upper internal pentagon near-side double-tip source', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = withStrokeDiagnosticsMode('full', () =>
        buildConstrainedSolidStrokeResolvedPackets(
          `self-check-star-solid-domain:inside:${joinType}:right-upper-near-side-tip-source`,
          topology.normalizedPoints,
          true,
          [
            createDefaultStroke({
      ...
      const sparseNearSideTipVertices =
        getRightUpperInsidePentagonNearSideTipVerticesForTest({
          legalFaceBoundaries,
          strokeWidth: 10,
          layers: [
            {
          ...
          (entry) =>
            entry.layer !== 'sourceStroke' && entry.layer !== 'renderClip'
        )

      expect(packets.length).toBeGreaterThan(0)
      expectRenderEntryHasSourceStrokeMaskForTest(entry)
      expect(
        sparseNearSideTipVertices,
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon near-side double-tip source`,
            reason:
              'right-upper internal corner must not expose one or two sparse product-visible side-tip vertices; renderClip and sourceStroke rows are diagnostic controls only',
            sparseNearSideTipVertices
          },
          null,
          2
  ...
  it('should run: reject detached right-upper internal pentagon render-clip islands inside cutouts', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-upper-detached-clip-island`,
        topology.normalizedPoints,
        true,
        [
          createDefaultStroke({
      ...
      const clipIslands = getDetachedPositiveIslandsInsideCutoutsForTest({
        polygons: clipPolygons,
        legalFaceBoundaries,
        strokeWidth: 10
      })
      const visibleIslands = getDetachedPositiveIslandsInsideCutoutsForTest({
        polygons: visiblePolygons,
        legalFaceBoundaries,
        strokeWidth: 10
      })
      ...
      expect(clipPolygons.length).toBeGreaterThan(0)
      expect(
        [...clipIslands, ...visibleIslands],
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon detached clip islands`,
            reason:
              'right-upper internal corner must be a single source-stroke join envelope; detached positive islands inside cutouts become the visible double-tip artifact at high zoom',
            clipIslands,
            visibleIslands
          },
          null,
  ...
  it('should run: preserve right-upper internal pentagon source-clip intersection coverage', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-upper-intersection-coverage`,
        topology.normalizedPoints,
        true,
        [
          createDefaultStroke({
      ...
      const sourceUnionPolygons = buildUnionPolygonsForTest(
        backend,
        sourcePolygons
      )
      const clipUnionPolygons = buildUnionPolygonsForTest(backend, clipPolygons)
      const samples = [-0.55, -0.475, 0.475, 0.5, 0.55].flatMap((angle) =>
        [8.5, 9.25, 10, 10.75, 11.5, 12.25].map((radius) => {
          const cos = Math.cos(angle)
          const sin = Math.sin(angle)
          const direction = {
            x: probe.probeDirection.x * cos + probe.sideDirection.x * sin,
          ...
          return {
            ...
            radius,
            point,
            sourceRaw: polygonListContainsPointWithWinding(
              sourcePolygons,
              point
            ),
            sourceUnion: polygonListContainsPointWithWinding(
              sourceUnionPolygons,
              point
            ),
            clipRaw: polygonListContainsPointWithWinding(clipPolygons, point),
            clipUnion: polygonListContainsPointWithWinding(
              clipUnionPolygons,
              point
            ),
            visible: polygonListContainsPointWithWinding(
              visiblePolygons,
              point
            ),
            nearestSourceVertex: nearestPolygonVertexForTest(
      ...
      const underAdmittedByIntersection = samples.filter(
        (sample) => sample.sourceUnion && sample.clipUnion && !sample.visible
      )
      const sourceOnlyLimits = samples.filter(
        (sample) => !sample.sourceUnion && sample.clipUnion && !sample.visible
      )
      const clipOnlyLimits = samples.filter(
        (sample) => sample.sourceUnion && !sample.clipUnion && !sample.visible
      )

      expect(packets.length).toBeGreaterThan(0)
      expect(sourcePolygons.length).toBeGreaterThan(0)
      expect(clipPolygons.length).toBeGreaterThan(0)
      expect(visiblePolygons.length).toBeGreaterThan(0)
      expect(
        underAdmittedByIntersection,
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon source/clip coverage`,
            reason:
              'if normalized source-stroke and normalized face clip both contain a point, the visible source-clip intersection must contain it too',
            underAdmittedByIntersection,
            sourceOnlyLimits,
            clipOnlyLimits,
            samples
  ...
  it('should run: locate right-upper internal pentagon side-peak contributors', async () => {
    const {
      sourcePath,
      topology,
      fillRegions,
    ...
    cases.forEach((joinType) => {
      const packets = buildConstrainedSolidStrokeResolvedPackets(
        `self-check-star-solid-domain:inside:${joinType}:right-upper-side-peak-contributors`,
        topology.normalizedPoints,
        true,
        [
          createDefaultStroke({
      ...
      const analysis = getInsideSolidCornerMicroProfileAnalysisForTest({
        ...
        strokeWidth: 10,
        containsPoint: (point) =>
          polygonListContainsPointWithWinding(visiblePolygons, point)
      })
      const detachedClipIslands =
        getDetachedPositiveIslandsInsideCutoutsForTest({
          polygons: clipPolygons,
          legalFaceBoundaries,
          strokeWidth: 10
        })
      const detachedVisibleIslands =
        getDetachedPositiveIslandsInsideCutoutsForTest({
          polygons: visiblePolygons,
          legalFaceBoundaries,
          strokeWidth: 10
        })
      const sidePeakContributors =
        detachedClipIslands.length === 0 &&
        detachedVisibleIslands.length === 0 &&
        !analysis.sidePeaksExceedCenter &&
        analysis.transitionValleys.length === 0 &&
        analysis.splitRays.length === 0
          ? []
          : analysis.radialFrontier.flatMap((entry) => {
              if (
                Math.abs(entry.angle) <= 0.62 ||
                entry.farthestCoveredRadius <= analysis.centerPeak + 0.75
              ) {
                return []
              }
              const cos = Math.cos(entry.angle)
              ...
              return [
                {
                  angle: entry.angle,
                  farthestCoveredRadius: entry.farthestCoveredRadius,
                  centerPeak: analysis.centerPeak,
                  point,
                  clipContributors: getContainingPolygonDiagnosticsForTest(
                    clipPolygons,
                    point
                  ),
                  sourceContributors: getContainingPolygonDiagnosticsForTest(
                    sourcePolygons,
                    point
                  ),
                  sourceSegmentContributors:
                    getSourceSegmentStrokeContributorsForTest({
                      backend,
                      sourcePath,
                      join: joinType,
                      strokeWidth: 10,
                  ...
                  visibleContributors: getContainingPolygonDiagnosticsForTest(
                    visiblePolygons,
                    point
                  ),
                  nearestFaceOwnershipEdges:
                    getFaceOwnershipEdgeContributorsForTest({
                      faceOwnershipTrace:
      ...
      expect(
        sidePeakContributors,
        JSON.stringify(
          {
            name: `inside solid ${joinType} right-upper internal pentagon side peak contributors`,
            reason:
              'right-upper internal corner must not expose side peaks from edge-strip or source-stroke fragments beyond the join envelope',
            sidePeakContributors
          },
          null,
          2
