...
Preset packages provide working defaults, not hidden ownership. App authors
should be able to add behavior, change explicit relations, or remove and define
complete capabilities through framework contracts without patching internals or
inheriting unrelated preset assumptions.
...
4. Preset Customization Guarantee

- Presets provide default capabilities that help products start quickly.
- Preset behavior must stay optional and removable by app or product owners.
- Structural changes use explicit relation removal/definition. Complete
  implementation changes use owner unregister followed by ordinary
  definition/registration; no API claims semantic equivalence.
...
- Preset packages own default wiring only when that wiring is optional and
  removable through the documented owner API.
...
- preset defaults that cannot be disabled, structurally changed, or safely
  unregistered
...
1. Can the behavior be added, structurally changed, or fully unregistered and
   defined through documented public APIs?
